...
enum BrowserCookieReader {
    ...
    enum ImportError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .keychainDenied(let svc):
                return "Keychain access denied for \(svc). Grant access in System Settings → Privacy."
            case .keychainNotFound(let svc):
                return "No keychain entry found for \(svc)."
            case .dbCopyFailed(let path):
                return "Failed to copy cookie database at \(path)."
            case .dbOpenFailed(let path):
                return "Failed to open cookie database: \(path)"
            case .noCookiesInDB:
                return "No claude.ai cookies found. Log in to claude.ai first."
            case .decryptionFailed(let found):
                return "Found \(found) cookie(s) but decryption failed."
            case .missingCookie(let hasSession, let hasOrg):
                let missing = !hasSession && !hasOrg ? "session key and org ID"
                    : !hasSession ? "session key" : "organization ID"
                return "Missing \(missing). Log in to claude.ai and try again."
            }
        }
    }
    ...
    private static func getDecryptionKey(service: String) -> Result<Data, ImportError> {
        ...
        guard let passwordString = String(data: passwordData, encoding: .utf8) else {
            return .failure(.keychainDenied("\(service) (invalid encoding)"))
        }
        ...
        guard derivationStatus == kCCSuccess else {
            return .failure(.keychainDenied("\(service) (PBKDF2 failed)"))
        }
